...
void main(int argc, char** argv) {
	ayqu_error_code_t e = AYQU_ERROR_NONE;
	ayqu_stream_t* stream = NULL;
	e = ayqu_create(&stream); ayqu_assert(e);
	e = ayqu_option(stream, AYQU_OPTION_ALLOCATOR, malloc, free); ayqu_assert(e);
	e = ayqu_option(stream, AYQU_OPTION_ENCODING, AYQU_ENCODING_UTF8); ayqu_assert(e);
	e = ayqu_option(stream, AYQU_OPTION_NOISE_TOLERANT, true); ayqu_assert(e);
	e = ayqu_callback(stream, AYQU_CALLBACK_ALL, callback); ayqu_assert(e);
	const char* json = "\"string value I am\"null{key: value}-1 NaN [1, 2, 3]";
	e = ayqu_feed(stream, json, strlen(json)); ayqu_assert(e);
	e = ayqu_dispose(stream); ayqu_assert(e);
}
